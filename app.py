...
class App:
    def __init__(self, db, data_formatter, artist_controller, transcribe_controller, transcriber):
        self.db = db
...
def before_request_func():
    ...
    if jwt.decode(request.headers["Authorization"], os.getenv("JWT_SECRET"), algorithms=["HS256"])["secret"] != os.getenv("APP_SECRET"):
        return jsonify({"error": "Unauthorized"}), 403
...
if __name__ == "__main__":
    ...
    app_instance = App(db, data_formatter, artist_controller, transcribe_controller, transcriber)
