...
class ArtistController:
    ...
    def get_artist(self, pseudonym: str):
        ...
        print(artist_data)
        if not artist_data:
            return self.res_formatter.with_errors("Artist not found").with_message("failure").with_status(404).with_additional_data({"pseudonym": pseudonym}).response()
        else:
            artist = Artist(db=self.db, **artist_data)
            artist_dict = artist.to_dict()
            artist_dict["_id"] = str(artist_data["_id"])
    ...
    def create_artist(self, request: json) -> jsonify:
        if request.json and "pseudonym" not in request.json:
            return self.res_formatter.with_errors("No pseudonym provided").with_status(400).response()
        elif request.json and "fullname" not in request.json:
            return self.res_formatter.with_errors("No fullname provided").with_status(400).response()
        elif request.json and "genre" not in request.json:
            return self.res_formatter.with_errors("No genre provided").with_status(400).response()
            ...
            try:
                ...
                return self.res_formatter.with_message("Artist inserted successfully").with_status(201).response()
            except DuplicateKeyError:
                return self.res_formatter.with_errors("Pseudonym already exists").with_status(400).response()
        else:
            return self.res_formatter.with_errors("Invalid JSON format").with_status(400).response()
    ...
    def delete_artist(self, pseudonym: str):
        ...
        if not artist_data:
            return self.res_formatter.with_errors("Artist not found").with_status(404).response()
        else:
            artist = Artist(db=self.db, **artist_data)
            artist.delete()
            return self.res_formatter.with_message("Artist deleted successfully").with_status(200).response()
        
    def force_delete_all_artists(self):
        self.db.artist_data_collection.delete_many({})
        return self.res_formatter.with_message("All artists deleted successfully").with_status(200).response()
    ...
    def update_artist(self, pseudonym: str, request: json):
        ...
        if not artist_data:
            ...
            artist.save()
            return self.res_formatter.with_message("Artist updated successfully").with_status(200).response()