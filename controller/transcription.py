...
class TranscriptionController:
    ...
    def _validate_audio_file(self, audio_file):
        ...
        if not audio_file:
            return (
                self.res_formatter.with_errors("No audio file provided")
                .with_status(400)
                .response()
            )
        ...
        if (
            ...
            return (
                self.res_formatter.with_errors("Invalid audio file extension")
                .with_status(400)
                .response()
            )
        ...
        if len(audio_data) == 0:
            return (
                self.res_formatter.with_errors("Audio file is empty")
                .with_status(400)
                .response()
            )
        if len(audio_data) > 100 * 1024 * 1024:  # 100 MB limit
            return (
                self.res_formatter.with_errors("Audio file is too large")
                .with_status(400)
                .response()
            )
        if audio_file.content_type not in [
            ...
            return (
                self.res_formatter.with_errors("Invalid audio file format")
                .with_status(400)
                .response()
            )
    ...
    def _validate_lyrics_file(self, lyrics_file):
        ...
        if lyrics_file.content_type != "text/plain":
            return (
                self.res_formatter.with_errors("Lyrics file must be a text file")
                .with_status(400)
                .response()
            )
        if (
            ...
            return (
                self.res_formatter.with_errors("Lyrics file must have a .txt extension")
                .with_status(400)
                .response()
            )
        return None
    ...
    def approve_transcription(self, artist: str, album: str, title: str, version: int):
        ...
        try:
            ...
            moved_files = self.storage_repository.move_approved_transcription(
